...
static unsigned getFixupKindLog2Size(const MCFixup &Fixup) {
  switch (Fixup.getKind()) {
  default:
    report_fatal_error("log2size(Fixup): Unhandled fixup kind!");
  case FK_Data_1:
    return 0;
  case FK_Data_2:
    return 1;
...
static unsigned getRelocType(const MCFixup &Fixup,
  ...
  if (IsPCRel) { // relative to PC
    switch (Fixup.getKind()) {
    default:
      format("%i", Fixup.getKind()).print(PrintBuffer, sizeof(PrintBuffer));
      report_fatal_error(
        Twine("Unsupported relative fixup kind \"") + PrintBuffer +
          "\""
      );
    ...
    case PPC::fixup_ppc_half16:
      switch (Spec) {
      default:
        format("%i", Spec).print(PrintBuffer, sizeof(PrintBuffer));
        report_fatal_error(
          Twine("Unsupported relocation type \"") + PrintBuffer +
            "\" for half16 fixup"
        );
      case PPC::S_None:
...
static uint32_t getFixupOffset(const MCAssembler &Asm,
                               const MCFragment *Fragment,
                               const MCFixup &Fixup) {
...
bool PPCMachObjectWriter::recordScatteredRelocation(
  ...
  const unsigned IsPCRel = Fixup.isPCRel();
  unsigned Type = getRelocType(Fixup, Target, IsPCRel);
